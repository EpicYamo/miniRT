...
t_vec3	get_object_center(t_scene *scene, int type, int index)
{
	...
	if (type == OBJ_TRIANGLE)
		return (vec3_create(scene->triangle_data[index].pos_x,
				scene->triangle_data[index].pos_y,
				scene->triangle_data[index].pos_z));
...
void	set_object_center(t_scene *scene, int type, int index, t_vec3 pos)
{
	...
	{
		scene->triangle_data[index].pos_x = pos.x;
		scene->triangle_data[index].pos_y = pos.y;
		scene->triangle_data[index].pos_z = pos.z;
	}
}
...
t_vec3	get_object_direction(t_scene *scene, int type, int index)
{
	...
	if (type == OBJ_TRIANGLE)
		return (vec3_create(scene->triangle_data[index].vector_x,
				scene->triangle_data[index].vector_y,
				scene->triangle_data[index].vector_z));
...
void	set_object_direction(t_scene *scene, int type, int index, t_vec3 dir)
{
	...
	{
		scene->triangle_data[index].vector_x = dir.x;
		scene->triangle_data[index].vector_y = dir.y;
		scene->triangle_data[index].vector_z = dir.z;
	}
}
